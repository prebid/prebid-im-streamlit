# streamlit_app.py  – Prebid Integration Monitor  (compact feed default, bug-fixed)
...
CSVGZ_URL = PARQUET_URL.replace(".parquet", ".csv.gz")  # fallback
...
# -------------------------------------------------
# Compact ↔ slim conversion (robust decoding)
# -------------------------------------------------
...
def compact_df_to_slim(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    for _,row in df.iterrows():
        modules  = _decode(row["modules"])
        libraries= _decode(row["libraries"])
        globals_ = _decode(row["globals"])
        version  = _decode(row["version"]) or None
        pb_inst  = int(row["pb_inst"]) if not pd.isna(row["pb_inst"]) else 0

        out.append({
            "siteKey": _decode(row["siteKey"]),
            "version": version,
            "modules": modules.split("|")   if modules else [],
            "libraries": libraries.split("|") if libraries else [],
            "globals": globals_.split("|")  if globals_ else [],
            "prebidInstances": [{}]*pb_inst,
        })
    return out
...
# -------------------------------------------------
# Default data loader (compact Parquet)
# -------------------------------------------------
...
with st.spinner("Loading data …"):
    if upload:
        ...
        if upload.name.endswith((".parquet",".csv",".gz",".csv.gz")):
            rows=read_compact_bytes(data_bytes, upload.name)
        else:
            rows=[slim_item(x) for x in jloads(data_bytes)]
    else:
        rows=load_remote_compact()

rows=[r for r in rows if len(r["modules"])<=MAX_MODS]
if not rows: st.warning("No data after filtering."); st.stop()
...
comp_bytes,comp_name,comp_mime = write_compact_file(rows)
...
def class_mod(m:str)->str:
    if "BidAdapter" in m: return "Bid Adapter"
    if any(x in m for x in ("RtdProvider","rtdModule")): return "RTD Module"
    if any(x in m for x in ("IdSystem","userId")): return "ID System"
    if any(x in m for x in ("Analytics","analyticsAdapter")): return "Analytics Adapter"
    return "Other"
...
vers_df=pd.Series([cat_version(d["version"]) for d in rows])\
...
mod_site,mod_inst=mod_stats(rows)
...
st.markdown(
    "<br><center>Feedback? "
    "<a href='mailto:chuie@prebid.org'>chuie@prebid.org</a></center>",
    unsafe_allow_html=True)
